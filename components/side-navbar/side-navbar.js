...
export const SideNavbar = (props) => {
    ...
    const getNavBarItem = (name, idx) => {
        // setActiveIndex(idx);
        return (
            <li key={idx} className="nav-item" onClick={() => activeSelectedNavItem(idx)}>
                <a href="#" className={props.showSideNavBar ? "nav-link active" : "nav-link text-white"} aria-current="page">
    ...
    const getInActiveNavBarItem = (value, idx) => {
        return (
            <li key={idx} className="nav-item"  onClick={() => activeSelectedNavItem(idx)} >
                <a  className={value.isSelected  ? "nav-link active" : "nav-link text-white"}>

                        {props.showSideNavBar? value.displayName : "...."}
    ...
    return (
        <div className="d-flex flex-column flex-shrink-0 p-3 text-bg-dark" style={{height: "100vh"}}>
            <div className="d-flex align-items-center mb-3 mb-md-0 me-md-auto text-white text-decoration-none">
                <Image src="/icons/1.png" onClick={()=>props?.setShowSideNavBar(!props?.showSideNavBar)}
                       width="50px"
                       height="50px"
                       alt="Expand"
                />
                {props?.showSideNavBar &&<span className="ps-2 fs-4">Mint your Stuff</span>}
            </div>
            <hr/>
